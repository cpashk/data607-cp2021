...
## Construct a path and load the file
...
## Create some useful values for our dataframe
...
## Start splitting the raw input into fields
...
## We'll use the pivot_longer to turn out individual round columns into rows, which will make joining data easier later on
...
## Now that we've got most of our data, we can start using calculations
...
## Since the instructions specify that we're interested in two separate groups, we'll split output
...
head(summarized_data %>% arrange(desc(expected_pt_diff)), 1)
```
